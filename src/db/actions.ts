  ...
  try {
    if (ownId) {
      ...
      const rows = await sql`
        ...
        VALUES (${ownId}, ${name}, ${address}, ${idProof || null}, ${idNumber || null}, ${dob || null}, ${mobile}, ${mobileAlt || null})
        RETURNING *
      `;
...
export async function updateCustomer(id: number, data: any) {
  ...
  try {
    await sql`
        UPDATE customers 
        SET own_id = ${ownId || null}, name = ${name}, address = ${address}, id_proof = ${idProof}, id_number = ${idNumber}, dob = ${dob}, mobile_no = ${mobile}, mobile_alt = ${mobileAlt || null}
        WHERE id = ${id}
      `;
...
export async function restoreCustomer(id: number) {
  return await sql`UPDATE customers SET is_deleted = FALSE, deleted_at = NULL WHERE id = ${id}`;
}
...
export async function getCustomerDetails(id: number) {
  try {
    ...
    return { customer, activeLoan, collections, allLoans, allCollections };
      ...
      return { customer: null, activeLoan: null, collections: [], allLoans: [], allCollections: [] };
  }
}
  ...
  try {
    const activeLoans = await sql`
      ...
      const totalCollectionsRows = await sql`
        ...
        FROM collections
        WHERE loan_id IN (SELECT id FROM loans WHERE status = 'active')
        GROUP BY loan_id
      `;
      const totalCollectedMap = new Map(totalCollectionsRows.map((r: any) => [r.loan_id, parseFloat(r.total)]));
    
      let result = activeLoans
        .filter((loan: any) => new Date(loan.start_date) <= new Date(dateStr))
        .map((loan: any) => ({
        loanId: loan.id,
      ...
      console.error("getCollectionStatus error:", e);
      return [];
  }
}
...
export async function getLedgerEntries() {
  try {
    return await sql`SELECT * FROM ledger ORDER BY date DESC, created_at DESC LIMIT 50`;
      ...
      console.error("getLedgerEntries error:", e);
      return [];
  }
}
...
export async function getFinancialSummary() {
  try {
    ...
    return { totalCollected, totalRotated, totalExpenses, totalCapital, initialBalance, expectedCash };
  } catch (e) {
      console.error("getFinancialSummary error:", e);
      return { totalCollected: 0, totalRotated: 0, totalExpenses: 0, totalCapital: 0, initialBalance: 0, expectedCash: 0 };
  }
}
